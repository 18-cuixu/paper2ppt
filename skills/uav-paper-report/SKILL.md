...
- Requirement variants must keep a stable source-paper identity for formula lookup. If smoke generation creates ids such as `primitive-planner-2025-brief`, formula helpers must use `base_id` or an equivalent original paper id before falling back to raw `formula_rows`; otherwise editable formulas silently regress to plain strings such as `p0`, `v0t`, or `t^2`.
- PPTX audits must reject raw formula notation in normal baseline formula runs, including caret exponents and code-style suffixes such as `p0`, `v0t`, `λc`, `λs`, `GSD0`, `Ltrack`, or `umin`. The audit should inspect run baseline so true superscript/subscript runs are accepted while plain-text formula shortcuts fail.
